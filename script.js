/*
    PredictPro V8 - Frontend Client (Final Ready-to-Push Version)
...
document.addEventListener('DOMContentLoaded', () => {
    const App = {
        ...
        state: {
            ...
            storeProducts: [], // Cache store products locally
            settings: {}, // Cache settings locally
        },
        
        // ** FINAL FIX: Dynamically determine API URL **
        // This is the most stable way to deploy on Render/Vercel.
        API_URL: window.location.protocol + "//" + window.location.host.replace('www.', '') + '/api',

        // --- 2. CORE APP INIT ---
        init: () => {
            // Check if a token is saved from a previous session
        ...
        handlers: {
            auth: {
                ...
                sendResetLink: async () => {
                    ...
                    App.ui.hideLoader();
                    App.ui.modal.hide();
                    // Show success even if email is fake for security reasons (to prevent user enumeration)
